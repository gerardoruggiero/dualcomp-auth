...
{
    public class CreateAddressTypeCommandHandler : ICommandHandler<CreateAddressTypeCommand, CreateAddressTypeResult>
    {
        private readonly IAddressTypeRepository _repository;
        private readonly IUnitOfWork _unitOfWork;

        public CreateAddressTypeCommandHandler(IAddressTypeRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository ?? throw new ArgumentNullException(nameof(repository));
            _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
        }

        public async Task<CreateAddressTypeResult> Handle(CreateAddressTypeCommand command, CancellationToken cancellationToken)
        {
            ...
            // Crear entidad
            var entity = AddressTypeEntity.Create(command.Name, command.Description);

            // Guardar entidad
            await _repository.AddAsync(entity, cancellationToken);
            await _unitOfWork.SaveChangesAsync(cancellationToken);

            return new CreateAddressTypeResult(entity.Id, entity.Name, entity.Description, entity.IsActive);
        }
