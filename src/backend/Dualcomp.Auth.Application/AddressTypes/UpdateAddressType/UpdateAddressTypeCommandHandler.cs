...
{
    public class UpdateAddressTypeCommandHandler : ICommandHandler<UpdateAddressTypeCommand, UpdateAddressTypeResult>
    {
        private readonly IAddressTypeRepository _repository;
        private readonly IUnitOfWork _unitOfWork;

        public UpdateAddressTypeCommandHandler(IAddressTypeRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository ?? throw new ArgumentNullException(nameof(repository));
            _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
        }

        public async Task<UpdateAddressTypeResult> Handle(UpdateAddressTypeCommand command, CancellationToken cancellationToken)
        {
            ...
            if (entity == null)
                throw new InvalidOperationException($"AddressType with ID {command.Id} not found");
            ...
            return new UpdateAddressTypeResult(entity.Id, entity.Name, entity.Description, entity.IsActive);
        }
        ...
        private async Task ValidateNameUniqueness(string name, Guid excludeId, CancellationToken cancellationToken)
        {
            ...
            if (existing.Any())
            {
                throw new InvalidOperationException($"An AddressType with name '{name}' already exists");
            }
        }
    }
}
