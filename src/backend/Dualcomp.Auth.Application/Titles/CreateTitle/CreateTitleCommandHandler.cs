...
{
    public class CreateTitleCommandHandler : ICommandHandler<CreateTitleCommand, CreateTitleResult>
    {
        private readonly ITitleRepository _repository;
        private readonly IUnitOfWork _unitOfWork;

        public CreateTitleCommandHandler(ITitleRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository ?? throw new ArgumentNullException(nameof(repository));
            _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
        }

        public async Task<CreateTitleResult> Handle(CreateTitleCommand command, CancellationToken cancellationToken)
        {
            ...
            // Crear entidad
            var entity = TitleEntity.Create(command.Name, command.Description);

            // Guardar entidad
            await _repository.AddAsync(entity, cancellationToken);
            await _unitOfWork.SaveChangesAsync(cancellationToken);

            return new CreateTitleResult(entity.Id, entity.Name, entity.Description, entity.IsActive);
        }
