...
export class SocialMediaComponent implements OnInit {
  ...
  items = signal<SocialMediaTypeEntity[]>([]);
  totalCount = signal(0);
  ...
  selectedItem = signal<SocialMediaTypeEntity | null>(null);
  ...
  tableConfig: DataTableConfig = {
    ...
    emptyMessage: 'No hay tipos de red social disponibles'
  };

  // Configuración del modal
  modalConfig: TypeModalConfig = {
    title: 'Tipo de Red Social',
    nameLabel: 'Nombre del Tipo',
    descriptionLabel: 'Descripción',
    namePlaceholder: 'Ej: Facebook, Instagram, LinkedIn',
    descriptionPlaceholder: 'Descripción del tipo de red social',
  ...
  loadDataFromServer() {
    ...
    this.socialMediaTypeService.getTypes(
      ...
      next: (result: SocialMediaTypeListResult) => {
        console.log('SocialMediaComponent - Datos recibidos del servidor:', result);
      ...
      error: (error) => {
        ...
        this.errorMessage.set('Error al cargar los tipos de red social');
        this.isLoading.set(false);
        this.isSearching.set(false);
      }
    });
  ...
  onEditType(item: SocialMediaTypeEntity) {
    console.log('SocialMediaComponent - Editando tipo:', item);
    // Cargar datos frescos del servidor antes de abrir el modal
    this.socialMediaTypeService.getTypeById(item.id).subscribe({
      next: (freshItem: SocialMediaTypeEntity) => {
        console.log('SocialMediaComponent - Datos frescos del servidor:', freshItem);
        this.selectedItem.set(freshItem);
        this.showModal.set(true);
      },
      error: (error) => {
        console.error('Error loading social media type for edit:', error);
  ...
  onModalSave(command: CreateSocialMediaTypeCommand | UpdateSocialMediaTypeCommand) {
    ...
    const observable = 'id' in command
      ? this.socialMediaTypeService.updateType(command.id, command)
      : this.socialMediaTypeService.createType(command);

    observable.subscribe({
      next: () => {
        console.log('SocialMediaComponent - Guardado exitoso, recargando datos del servidor...');
      ...
      error: (error) => {
        console.error('Error saving social media type:', error);
        this.modalLoading.set(false);
      }
    });
  }
